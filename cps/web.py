...
@web.route("/ajax/book/history", methods=['GET'])
@user_login_required
def get_book_history():
    bookId = request.args.get('bookId')
    userId = current_user.id
    book_history = ub.get_book_history(bookId, userId)
    if book_history:
        return jsonify({
            "code": 200,
            "data": {
                "bookId": bookId,                
                "historyKey": book_history.historyKey,
                "historyValue": book_history.historyValue
            }
        })
    else:
        return jsonify({
...
@web.route("/ajax/book/history", methods=['POST'])
@user_login_required
def add_book_history():
    try:
        reqData = request.get_json()
        bookId = reqData.get("bookId")
        bookType = reqData.get("bookType")
        userId = current_user.id
        historyKey = reqData.get("historyKey")
        historyValue = reqData.get("historyValue")
    except Exception:
        log.error("Could not parse request data: %r", reqData)
        return "", 400
        
    book_history = ub.set_book_history_daily(bookId, bookType, userId, historyKey, historyValue)
    if book_history:
        return jsonify({
            "code": 200,
            "data": json.loads(book_history.to_json())
        })
    else:
        log.error("Could not set book history: %r", reqData)        
        return "", 400
...
def read_book(book_id, book_format):
    ...
    book_history = ub.get_book_history(book_id,current_user.id)
    if book_history:
        book_history = {
            "historyKey": book_history.historyKey,
            "historyValue": book_history.historyValue,
    ...
    if book_format.lower() == "epub":
        ...
        return render_title_template('read.html', bookid=book_id, title=book.title, bookmark=bookmark, booktype=book_format.lower(), bookHistory=book_history)
    elif book_format.lower() == "pdf":
        log.debug("Start pdf reader for %d", book_id)
        return render_title_template('readpdf.html', pdffile=book_id, title=book.title, booktype=book_format.lower(),bookid=book_id, bookHistory=book_history)
    elif book_format.lower() == "txt":
        log.debug("Start txt reader for %d", book_id)
        return render_title_template('readtxt.html', txtfile=book_id, title=book.title, booktype=book_format.lower(),bookid=book_id, bookHistory=book_history)
    elif book_format.lower() in ["djvu", "djv"]:
        log.debug("Start djvu reader for %d", book_id)
        return render_title_template('readdjvu.html', djvufile=book_id, title=book.title,
                                     extension=book_format.lower(),booktype=book_format.lower(),bookid=book_id, bookHistory=book_history)
    else:
