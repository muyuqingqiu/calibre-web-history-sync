...
from sqlalchemy import String, Integer, SmallInteger, Boolean, DateTime, Float, JSON, TIMESTAMP
...
class Historys(Base):
    ...
    historyKey = Column(String(collation='NOCASE'), nullable=False)
    historyValue = Column(JSON(), nullable=False)
    bookType = Column(String(collation='NOCASE'), nullable=False)
    ts = Column(TIMESTAMP, default=datetime.datetime.utcnow)

    def __init__(self, bookId,bookType, userId, historyKey, historyValue):
        super().__init__()
        self.historyObject = {
    ...
    def get(self):
        return self.historyKey

    def __eq__(self, otherBookId, otherUserId):
        return self.bookId == otherBookId and self.userId == otherUserId        

    def __repr__(self):
        return "<History(bookId:{0},userId{1})>".format(self.bookId, self.userId)

    def to_json(self):
        content = dict()
        content[self.historyKey] = self.historyValue
        return json.dumps(content, ensure_ascii=False)
    
# 自定义方法
def get_book_history(bookId, userId):
    return session.query(Historys).filter(Historys.bookId == bookId, Historys.userId == userId).order_by(Historys.ts.desc()).first()

def get_user_book_historys(userId):
    return session.query(Historys).filter(Historys.userId == userId).order_by(Historys.ts.desc()).all()

def set_book_history(bookId, bookType, userId, historyKey, historyValue):
    history = Historys(bookId, bookType, userId, historyKey, historyValue)
    session.add(history)
    session.commit()
    return history

def set_book_history_daily(bookId, bookType, userId, historyKey, historyValue):
    history = get_book_history(bookId, userId)
    if history:
        if history.ts.date() == datetime.datetime.now().date():
            history.historyValue = historyValue
            session.commit()
            return history
        else:
            return set_book_history(bookId, bookType, userId, historyKey, historyValue)
    else:
        return set_book_history(bookId, bookType, userId, historyKey, historyValue)
                            
def delete_book_history(bookId, userId):
    session.query(Historys).filter(Historys.bookId == bookId, Historys.userId == userId).delete()
    session.commit()
    
def delete_book_historys(userId):
    session.query(Historys).filter(Historys.userId == userId).delete()
    session.commit()
